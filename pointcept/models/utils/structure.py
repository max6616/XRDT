...
class Point(Dict):
    ...
    def to(self, device, non_blocking=False):
        """
        Move all tensors in the Point object to the specified device.
        This is needed for training when using DataLoader with pin_memory=True.
        """
        new_point = Point()
        for key, value in self.items():
            if isinstance(value, torch.Tensor):
                new_point[key] = value.to(device, non_blocking=non_blocking)
            else:
                new_point[key] = value
        return new_point
